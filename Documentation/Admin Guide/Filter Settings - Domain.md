...
The **Administrator** can select default settings for the entire domain.  If the settings for a particular user are not overridden, they will use these settings.

Per-user settings are only available if you have selected the "Email Account list is complete" option on the ![Email Account List]().
